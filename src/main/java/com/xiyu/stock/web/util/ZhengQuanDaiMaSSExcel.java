...
public class ZhengQuanDaiMaSSExcel {
	...
	public List<InfoZhengQuanDaiMaSS> readInfo(String filename) throws FileNotFoundException, IOException {
		Workbook workbook = new XSSFWorkbook(new FileInputStream(filename));
		List<InfoZhengQuanDaiMaSS> list = new ArrayList<InfoZhengQuanDaiMaSS>();
		int num = workbook.getNumberOfSheets();
		for (int page = 0; page < num; page++) {
			Sheet sheet = workbook.getSheetAt(page);
